...
**`novolis-3d`** — packable `Novolis.ThreeD.Import.Assimp` (AssimpNet → `TriangleMesh` / `EditableMesh`).

Also: **`novolis-raylib`** `Novolis.Raylib.Loaders` remains OBJ-only (no Assimp).

### Status

- **Done:** `Novolis.ThreeD.Import.Assimp` (`AssimpMeshImporter`, `MeshImportOptions`), SceneLab `importmesh` + **Import…**, CorellianFreighterBuilder `--import` uses the package.
